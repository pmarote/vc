...
def gerar_rel_an_econ(cursor, out_path, debug=False):
    ...
    executar_e_formatar("""
        ...
        SELECT Part, 
               es_valconEFD, 
               ROUND((es_valconEFD * 100.0) / NULLIF(SUM(es_valconEFD) OVER (), 0), 2) AS "%",
               es_icmsEFD, es_icmsstEFD, es_valconDFe, es_icmsDFe, es_icmsstDFe
        FROM VisaoFinal 
    ...
    executar_e_formatar("""
        SELECT Part, 
          SUM(es_valconEFD) AS es_valconEFD, 
          ROUND((SUM(es_valconEFD) * 100.0) / NULLIF(SUM(SUM(es_valconEFD)) OVER (), 0), 2) AS "%",
          SUM(es_icmsEFD) AS es_icmsEFD, SUM(es_icmsstEFD) AS es_icmsstEFD,
          SUM(es_valconDFe) AS es_valconDFe, SUM(es_icmsDFe) AS es_icmsDFe, SUM(es_icmsstDFe) AS es_icmsstDFe
